...
@Command(name = "MakeMeasurementsFile", mixinStandardHelpOptions = true, version = "MakeMeasurementsFile 0.1",
        description = "Make Measurements file for running 1brc solutions against")
class MakeMeasurementsFile implements Callable<Integer> {

    @Parameters(description = "The file to create", defaultValue = "measurements.txt")
    private File measurementsFile;

    @Option(names = {"-c", "--count"}, description = "Number of measurements to create", defaultValue = "1000000000")
    private long measurementsCount;

    @Option(names = {"-f", "--force"}, description = "Overwrite existing measurements file", defaultValue = "false")
    private boolean isForceOverwrite;

    public static void main(String... args) {
        int exitCode = new CommandLine(new MakeMeasurementsFile()).execute(args);
        System.exit(exitCode);
    }

    @Override
    public Integer call() throws Exception {
        if (measurementsFile.exists() && !isForceOverwrite) {
            var fileSizeMegabytes = measurementsFile.length() / 1024 / 1024;
            System.err.printf("Measurements file '%s' (%dMB) already exists, use --force to overwrite%n",
                measurementsFile.getPath(), 
                fileSizeMegabytes);
            return 1;
        }

        System.out.printf("Creating %,d in file %s%n", measurementsCount, measurementsFile);
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(measurementsFile))) {
            for (int i = 0; i < measurementsCount; i++) {
                if (i > 0 && i % 50_000_000 == 0) {
                    System.out.printf("Wrote %,d measurements in %s ms%n", i, millisSince(start));
                }
        ...
        System.out.printf("Created file with %,d measurements in %s ms%n", measurementsCount, millisSince(start));

        return 0;
    }

    private static long millisSince(final long startTimeNanos) {
        var nanosSince = System.nanoTime() - startTimeNanos;
        return nanosSince / 1_000_000;
    }
